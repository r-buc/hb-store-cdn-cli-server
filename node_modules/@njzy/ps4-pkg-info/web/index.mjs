...
var createSeek = (url) => {
  ...
  const seek = async (offset, whence) => {
    ...
    username && password && headers.append("Authorization", "Basic " + Buffer2.from(username + ":" + password).toString("base64"));
  ...
  return {
    seekChunk: async (offset, whence) => {
      ...
      let totalChunk = Buffer2.from([]);
      while (true) {
        const { value, done } = await reader.read();
        if (done) {
          break;
        }
        totalChunk = Buffer2.concat([totalChunk, value]);
      }
      reader.releaseLock();
      return totalChunk;
    },
...
var getPs4PkgInfo = async (pkgFileUrl, options) => {

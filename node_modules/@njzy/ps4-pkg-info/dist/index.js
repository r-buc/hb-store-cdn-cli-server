...
__export(node_exports, {
  Ps4PkgAppType: () => Ps4PkgAppType,
  Ps4PkgCategory: () => Ps4PkgCategory,
  getPs4PkgInfo: () => getPs4PkgInfo
});
module.exports = __toCommonJS(node_exports);
var import_fs = __toESM(require("fs"));

// src/core.ts
var import_python_struct = __toESM(require("python-struct"));
...
var extract = async ({
  ...
  const totalTanleEntry = import_python_struct.default.unpack(">I", baseChunk.slice(16, 23 + 1))[0];
  const offsetTableEntry = import_python_struct.default.unpack(">I", baseChunk.slice(24, 31 + 1))[0];
    ...
    for (let i = 0; i < le32(paramSfoHeader.sectionTotal); i++) {
      ...
      switch (paramSfoSection.dataType) {
        case ParamSfoSectionDataType.string:
          value = paramSfoData.toString("utf-8", le32(paramSfoSection.dataOffset), le32(paramSfoSection.dataOffset) + le32(paramSfoSection.usedDataField) - 1);
          break;
        case ParamSfoSectionDataType.integer:
          value = le32(paramSfoData.slice(le32(paramSfoSection.dataOffset), le32(paramSfoSection.dataOffset) + le32(paramSfoSection.usedDataField)));
          break;
        default:
          break;
      }
  ...
  destroyAll == null ? void 0 : destroyAll();
  return {
    paramSfo: generateParamSfo ? res == null ? void 0 : res[0] : void 0,
    icon0Raw: !generateParamSfo ? res == null ? void 0 : res[0] : res == null ? void 0 : res[1]
  };
};
var getPkgTableEntrySturct = (chunk) => {
  const data = import_python_struct.default.unpack(">IIIIII8x", chunk);
  return {
    id: data[0],
...
var createSeek = (pkgFilePath) => {
  ...
  const seek = (offset, whence) => {
    const stream = import_fs.default.createReadStream(pkgFilePath, {
      start: offset,
      end: whence
    });
    streams.push(stream);
...
var getPs4PkgInfo = async (pkgFilePath, options) => {
  ...
  const res = await extract(__spreadValues({ seekChunk, destroyAll }, options));
  if ((res == null ? void 0 : res.icon0Raw) && (options == null ? void 0 : options.generateBase64Icon)) {
